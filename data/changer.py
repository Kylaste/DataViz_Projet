...
with open("regularite-mensuelle-intercites.csv", "r") as fichier:
    ...
    with open("trajets_intercites.csv", "w") as denFile:
        ...
        for row in cr:
                if( compteur > 0):
                    datetime_object = datetime.strptime(row[1], '%Y-%m')
                    id = row[3]+"-"+row[4]+"-"+str(datetime_object.year)
                    
                    if(row[9] != ""):
                        if( id in moyenne):
                            moyenne[id] += float(row[9])
                            nbMois[id] += 1
                        else:
                            moyenne[id] = float(row[9])
                            nbMois[id] = 1
                    fileWriter.writerow([row[3], row[4], row[9], datetime_object.year, datetime_object.month, ""])
                compteur = compteur + 1
        for row in moyenne:
